...
host="" # Ensure port provided for opensource clusters
workload="http_logs"
...
id="traditional_test_$(date +"%Y_%m_%d_%I_%M_%p")"
...
sudo docker run -v /home/ec2-user/benchmark-data/:/opensearch-benchmark/.benchmark/ opensearchproject/opensearch-benchmark:latest execute-test --workload=$workload --pipeline=benchmark-only --target-hosts=$host --telemetry=node-stats --test-execution-id=$id --user-tag=test-type:traditional-single-idx --workload-params=$workload_params > /home/ec2-user/benchmark-data/"$(date +"%Y_%m_%d_%I_%M_%p").log"
